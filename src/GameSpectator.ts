...
export class GameSpectator {
    ...
    private async endGame() {
        ...
        if (config.spectator.get_riot_match_details) {
            ...
            await fsp.writeFile(`${config.paths.games}/${this.region}/${this.game.gameId}/apiGameInfo.json`, JSON.stringify(apiGameInfo.response, null, 4));
        }
    ...
    private async saveEndOfGameStats() {
        ...
        try {
            ...
            await fsp.writeFile(`${config.paths.games}/${this.region}/${this.game.gameId}/endOfGameStats.json`, JSON.stringify(response.data, null, 4));
        } catch (error) {
